...
#import "JibenInfoViewController.h"
#import "MJRefresh.h"
...
@interface JibenInfoViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,ZLPhotoPickerBrowserViewControllerDataSource,ZLPhotoPickerBrowserViewControllerDelegate>
...
@implementation JibenInfoViewController
...
- (void)loadData
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [DownLoadImageObj getImageDataFromserverWithBlock:^(id respon, NSError *error) {
            ...
            if(!error)
            {
                for (DownLoadImageObj *downImageObj in respon) {
                    UIImage *image = [UIImage sd_imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[CailaiAPIImageURLString stringByAppendingString: downImageObj.urlString]]]];
                    if(image == nil)
                    {
        ...
        } withFilesType:@"1" withPid:_userID];
    });
